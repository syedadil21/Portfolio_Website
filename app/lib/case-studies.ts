...
export const caseStudies: CaseStudy[] = [
  {
    ...
    images: [
      { src: "/projects/ai-analytics/bonded-report.png", alt: "AI-generated analytics report — natural language query results with visualizations" },
      { src: "/projects/ai-analytics/bonded-report-2.png", alt: "Analytics dashboard — interactive report with data breakdown and charts" },
    ],
  ...
  },
  {
    ...
    images: [
      { src: "/projects/fbta/Dashboard_admin_redesigned.png", alt: "Redesigned admin dashboard — updated layout and management interface" },
      { src: "/projects/fbta/Dashboard_student_redesigned.png", alt: "Redesigned student dashboard — personalized course view and progress tracking" },
      { src: "/projects/fbta/Dashboard_student_redesigned_2.png", alt: "Redesigned student dashboard — additional view with lecture access and activity" },
    ...
    subtitle: "Video learning platform with DRM protection, custom security layers, and device verification",
    category: "Web",
    stack: [
      "Next.js",
      "Fastify",
    ...
    overview:
      "A full-stack video learning platform with course management and multi-layer content security. Originally built custom video infrastructure with HLS streaming via Backblaze B2, Cloudflare CDN, presigned URLs, and device fingerprinting. Migrated to VdoCipher for hardware-level DRM (Widevine/FairPlay) to prevent screen recording and screenshots — a protection level not achievable with custom software alone. Includes a real-time admin dashboard for session monitoring and security alerts.",
    problem: [
      "Video content needed to be delivered securely — preventing unauthorized downloads, screen recording, screenshots, and link sharing.",
      "Software-level protections (watermarks, DevTools detection, context menu disabling) could not prevent screen recording or screenshots — the client required hardware-level DRM enforcement.",
    ...
    solution: [
      "Initially built custom video infrastructure: HLS streaming via Backblaze B2 (S3-compatible) with Cloudflare CDN delivery, presigned URLs with 10-minute expiry, and Video.js player with custom security controls.",
      "Migrated to VdoCipher for DRM-based protection (Widevine for Chrome/Android, FairPlay for Safari/iOS) — providing hardware-level screen recording and screenshot prevention that custom software solutions cannot achieve. Made the integration decision and led the migration.",
      ...
      "Retained software-level protections as defense-in-depth: disabled context menu, hidden native controls, DevTools detection (pauses video), and rotating watermarks with user email/ID.",
    ...
    features: [
      "DRM protection via VdoCipher (Widevine/FairPlay) — blocks screen recording and screenshots",
      "Presigned URLs with 10-minute expiry (original Backblaze B2 infrastructure)",
    ...
    outcome: [
      "Migrated from custom video infrastructure (Backblaze B2 + Cloudflare + HLS) to VdoCipher DRM — achieving hardware-level screen recording and screenshot prevention that software-only solutions could not provide.",
      "Retained custom security layers (device fingerprinting, OTP verification, stream locking, watermarking) as defense-in-depth alongside DRM.",
      "Reduced operational overhead on video infrastructure while strengthening content protection.",
    ...
    role: "Full stack developer — designed the multi-layer security architecture, built original video infrastructure, made the VdoCipher integration decision and led the DRM migration, implemented device verification, stream locking, admin dashboard, Fastify API, and Next.js frontend.",
  },
  {
  ...
  },
  {
    ...
    images: [
      { src: "/projects/bento/IMG_5864.PNG", alt: "Bento app — voice assistant UI with animated halo visualizer" },
      { src: "/projects/bento/IMG_5865.PNG", alt: "Bento app — recipe screen with AI cooking assistant" },
      { src: "/projects/bento/xrecorder-16032025-073115_tVPsllAo.mov", alt: "Demo — user flow for scanning produts and adding/subtracting items on the basis of the results" },
      { src: "/projects/bento/XRecorder_23042025_190733.mov", alt: "Demo — real-time voice visualizer responding to microphone input" },
    ],
    subtitle: "Recipe app with ingredient scanning, voice visualizer, ESP-32 hardware integration, and AI assistant",
    category: "AI",
    stack: ["React Native (Expo)", "Reanimated", "Audio API", "ESP-32", "Bluetooth", "Image Recognition"],
    overview:
      "A food recipe application with an AI-powered cooking assistant (ElevenLabs voice agent on ESP-32 hardware). Built real-time voice visualization, a swipe-based ingredient confirmation flow from camera scans, Bluetooth hardware provisioning, and contributed to UI redesign. Team project.",
    problem: [
      ...
      "Users needed a quick way to add fridge/pantry ingredients to their inventory without manually entering each item — the process was tedious and error-prone.",
    ...
    solution: [
      ...
      "Designed and built a swipe-based ingredient confirmation flow — users photograph their fridge/pantry items, the app scans and identifies ingredients with quantities, then presents each item as a card for the user to swipe right (confirm) or left (reject) before adding to inventory. Eliminates manual data entry for inventory management.",
    ...
    features: [
      "Camera-based ingredient scanning with image recognition",
      "Swipe-to-confirm ingredient cards (right to add, left to reject)",
      "Automated inventory management from photo scans",
    ...
    outcome: [
      "Ingredient scanning with swipe confirmation turned a tedious manual inventory process into a quick photo-and-swipe workflow.",
    ...
    role: "Contributor (team project) — designed the swipe-based ingredient scanning flow, built the real-time voice visualizer, implemented Bluetooth Wi-Fi provisioning for ESP-32, and contributed to UI redesign.",
  },
];
